...
@interface CSNumberApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation CSNumberApi

NSString* kCSNumberApiErrorDomain = @"CSNumberApiErrorDomain";
NSInteger kCSNumberApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) numbersBuyByDedicatedNumberPostWithDedicatedNumber: (NSString*) dedicatedNumber
    ...
    if (dedicatedNumber == nil) {
        NSParameterAssert(dedicatedNumber);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"dedicatedNumber"] };
            NSError* error = [NSError errorWithDomain:kCSNumberApiErrorDomain code:kCSNumberApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/numbers/buy/{dedicated_number}"];

    NSMutableDictionary *pathParams = [[NSMutableDictionary alloc] init];
    if (dedicatedNumber != nil) {
        pathParams[@"dedicated_number"] = dedicatedNumber;
    }
...
-(NSURLSessionTask*) numbersGetWithPage: (NSNumber*) page
    ...
    completionHandler: (void (^)(NSString* output, NSError* error)) handler {
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/numbers"];
...
-(NSURLSessionTask*) numbersSearchByCountryGetWithCountry: (NSString*) country
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/numbers/search/{country}"];
