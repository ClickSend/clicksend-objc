...
@interface CSSubaccountApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation CSSubaccountApi

NSString* kCSSubaccountApiErrorDomain = @"CSSubaccountApiErrorDomain";
NSInteger kCSSubaccountApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) subaccountsBySubaccountIdDeleteWithSubaccountId: (NSNumber*) subaccountId
    ...
    if (subaccountId == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kCSSubaccountApiErrorDomain code:kCSSubaccountApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/subaccounts/{subaccount_id}"];
...
-(NSURLSessionTask*) subaccountsBySubaccountIdPutWithSubaccountId: (NSNumber*) subaccountId
    ...
    if (subaccount == nil) {
        NSParameterAssert(subaccount);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"subaccount"] };
            NSError* error = [NSError errorWithDomain:kCSSubaccountApiErrorDomain code:kCSSubaccountApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
...
-(NSURLSessionTask*) subaccountsGetWithPage: (NSNumber*) page
    ...
    completionHandler: (void (^)(NSString* output, NSError* error)) handler {
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/subaccounts"];
...
-(NSURLSessionTask*) subaccountsRegenApiKeyBySubaccountIdPutWithSubaccountId: (NSNumber*) subaccountId
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/subaccounts/{subaccount_id}/regen-api-key"];
