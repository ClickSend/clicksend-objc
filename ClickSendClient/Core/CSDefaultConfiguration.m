...
#import "CSBasicAuthTokenProvider.h"
#import "CSLogger.h"

@interface CSDefaultConfiguration ()
...
@implementation CSDefaultConfiguration

#pragma mark - Singleton Methods

+ (instancetype) sharedConfig {
    static CSDefaultConfiguration *shardConfig = nil;
...
- (instancetype) init {
    self = [super init];
    if (self) {
        _host = @"https://rest.clicksend.com/v3";
        _username = @"";
        _password = @"";
        _accessToken= @"";
        _verifySSL = YES;
...
- (NSString *) getBasicAuthToken {
    ...
    NSString *basicAuthToken = [CSBasicAuthTokenProvider createBasicAuthTokenWithUsername:self.username password:self.password];
